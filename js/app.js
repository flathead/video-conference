...
function onConnectionEstablished() {
  ...
  var $authForm = $('<div id="authForm">'+
    ...
    '<div class="form-group">'+
      '<label for="inputUsername" class="col-sm-2 control-label">Имя пользователя</label>'+
      '<div class="col-sm-10">'+
...
// Login function
function login() {
    log(username+"@"+application_name+"."+"miles.n4.voximplant.com");
  voxAPI.login(username+"@"+application_name+"."+"miles.n4.voximplant.com", password);
...
function onAuthResult(e) {
  log("Авторизация: "+e.result);
  if (e.result) { 
    // Authorized successfully         
    dialog.close();
    var title = $('.panel-title').html() + ': залогинился как ' + username;
    $('.panel-title').html(title);
    $('#controls').slideDown();
    showLocalVideo(true);      
  } else {
    // Wrong username or password
    if (!$('div.alert.alert-danger').length) $('#authForm').prepend('<div class="alert alert-danger" role="alert">Был указан неверный логин или пароль</div>');
    log("Code: "+e.code);
  }
}
...
function onLocalVideoStream(e) {
  console.log("ЛОКАЛЬНАЯ ВИДЕОТРАНСЛЯЦИЯ");
  console.log(e);
  if (e.type == "sharing") {
...
function onCallConnected(e) {          
  ...
  if ($('#cancelButton').length) {
    $('#cancelButton').html('Отключить');
    $('<button type="button" class="btn btn-default" id="shareButton">Поделиться экраном</button>').insertAfter("#cancelButton");
  } else {    
    $('#callButton').replaceWith('<button type="button" class="btn btn-danger" id="cancelButton">Отключить</button>');    
    $('<button type="button" class="btn btn-default" id="shareButton">Поделиться экраном</button>').insertAfter("#cancelButton");
...
function onCallDisconnected(e) {
  log("ВызовОтключен: "+currentCall.id()+" Состояние вызова: "+currentCall.state());
  currentCall = null;
  $('#cancelButton').replaceWith('<button type="button" class="btn btn-success" id="callButton">Вызов</button>');
...
function onMicAccessResult(e) {
  log("Доступ к микрофону/камере разрешен: "+e.result);
  if (e.result) {       
    // Access was allowed   
    ...
    dialog.setMessage('Вы должны разрешить доступ к вашему микрофону, чтобы использовать сервис');
  }
}
...
function onIncomingCall(e) {
  ...
  log("Входящий звонок от: "+currentCall.number());
...
function onProgressToneStart(e) {
  log("ProgessToneStart для идентификатора вызова: "+currentCall.id()); 
}

// Progres tone play stop
function onProgressToneStop(e) {
  log("ProgessToneStop для идентификатора вызова: "+currentCall.id());  
}

// Create outbound call
function createCall() {
  $('#callButton').replaceWith('<button type="button" class="btn btn-danger" id="cancelButton">Отмена</button>');
  $('#callButton').remove();
  $('#cancelButton').click(function() {
    currentCall.hangup();
  });
...
function connect() {
  ...
  if (mode == 'webrtc' && voxAPI.isRTCsupported()) {
    dialog = new BootstrapDialog({
      title: 'Доступ к камере/микрофону',
      message: 'Пожалуйста, нажмите Разрешить, чтобы разрешить доступ к вашей камере и микрофону.',
      closable: false      
    });
    dialog.open();  
  }
